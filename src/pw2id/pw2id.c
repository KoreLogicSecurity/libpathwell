 ...
 ***********************************************************************
 *
 * $Id: pw2id.c,v 1.31.2.1 2015/09/30 16:05:54 klm Exp $
 *
 ***********************************************************************
 *
...
main(int iArgumentCount, char *ppcArgumentVector[])
{
  ...
  while ((c = getopt(iArgumentCount, ppcArgumentVector, "f:s:t:")) != -1)
  {
    switch (c)
    {
    case 'f':
  ...
  if (optind != iArgumentCount)
  {
    for (iIndex = optind; iIndex < iArgumentCount; iIndex++)
    {
      ...
      {
        printf("Password='%s'; ID='%"PRId64"';\n", PwTGetPassword(psProperties->psPwTContext), PwTGetId(psProperties->psPwTContext));
      }
    }
  }
...
{
  fprintf(stderr, "\n");
  fprintf(stderr, "Usage: %s [-f {file|-}] [-s token-set] [-t encoding-type] [password [...]]\n", PROGRAM_NAME);
  fprintf(stderr, "\n");
